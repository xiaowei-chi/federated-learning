...
def combine_category(graphs, category_split):
    ...
    for i in range(len(category_split)):
        ls = category_split[i]
        print("combining subgraphs for " + str(i) + " client")
        graph_new = graphs[ls[0]]
        for i in range(1, len(ls)):
            print("combined " + str(i + 1) + " subgraphs")
...
def get_data_category(args, path, data, load_processed=True):
    ...
    assert args.num_users <= len(graphs)

    perm = list(torch.randperm(len(graphs)))
    category_split = np.array_split(perm, args.num_users)
    graphs = combine_category(graphs, category_split)

    print("---------------converting to node degree-----------------")
    graphs = _convert_to_nodeDegreeFeatures(graphs)
    print(graphs)
    graphs_split = []
    print("---------------spliting into trian val and test-----------")
    for g in graphs:
